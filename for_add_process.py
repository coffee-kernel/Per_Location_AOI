...
def correction(
        ...
        color_percent, saturation_percent = 1.0
):
    """
    ...
    :param saturation_percent [-1.0 ~ 1.0]: Adjust saturation (-1.0 decreases fully, 1.0 increases fully).
    :return: Processed RGB image numpy array.
    """
    ...
    # Adjust global saturation using HSV space
    if saturation_percent != 0:
        hsv = cv2.cvtColor(output, cv2.COLOR_BGR2HSV).astype(float)
        h, s, v = cv2.split(hsv)
        s = np.clip(s * (1 + saturation_percent), 0, 255)
        hsv = cv2.merge([h, s, v])
...
if __name__ == "__main__":
    img_path = "./Test_images/IMG_1417.JPG"
    img = cv2.imread(img_path)

    if img is None:
        raise FileNotFoundError(f"Could not load image at {img_path}")
    
    shadow_amount_percent = 0.0
    shadow_tone_percent = 0.5
    shadow_radius = 30
    highlight_amount_percent = 1.0
    highlight_tone_percent = 0.7
    highlight_radius = 30
    color_percent = 0.6
    saturation_percent = -0.3 

    result = correction(img, shadow_amount_percent, shadow_tone_percent, shadow_radius,
                        highlight_amount_percent, highlight_tone_percent, highlight_radius,
                        color_percent, saturation_percent)

    cv2.imwrite("flattened_PSlike_2.jpg", result)
